...
public class UnitTrackingContainer extends AppCompatActivity implements UnitTrackingView, View.OnClickListener, SearchView.OnQueryTextListener {
    ...
    private void updateUnitDB(List<Unit> unitList) {
        for (Unit unit : unitList) {
            UnitDB.updateUnits(unit.getId(), unit.isVehicleStatus(), unit.getCveVehicle(), unit.getVehicleSwitch(), unit.getVehicleName(), unit.getVehicleImage(),
                    unit.getSendTime(), unit.getDescBrand(), unit.getDescModel(), unit.getVehicleYear(), unit.getVehicleVin(), unit.getVehiclePlate(), unit.getGeoreference(),
                    unit.getTimeTravel(), unit.getTimeElapsed(), unit.getLatitude(), unit.getLongitude(), unit.getMileage(), unit.getKmTravel(), unit.getCurrentSpeed(), unit.getMaxSpeed(),
                    unit.getDesc_layer());
        }
    }
