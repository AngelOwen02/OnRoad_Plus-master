...
public class UnitAssignSupportInteractorImpl implements UnitAssignSupportInteractor {
    ...
    public UnitAssignSupportInteractorImpl(UnitAssignSupportPresenter presenter) {
    ...
    public void getUnitsfromgroups(Context context) {
        if(UnitAssignSupportContainer.integervalueforrequest!=0) {
            SharedPreferences preferences = context.getSharedPreferences(GeneralConstantsV2.CREDENTIALS_PREFERENCES, Context.MODE_PRIVATE);
            String token = preferences.getString(GeneralConstantsV2.TOKEN_PREFERENCES, null);
            //cveofgroup = 53;
            startGroupsRequestVehicles(UnitAssignSupportContainer.integervalueforrequest, token);
    ...
    private void createTemporalUnitDB(List<Unit> unitList) {
        for (Unit unit : unitList) {
            TemporalUnitDB.createNewUnit(unit.isVehicleStatus(), unit.getCveVehicle(), unit.getVehicleSwitch(), unit.getVehicleName(), unit.getVehicleImage(),
                    ...
                    unit.getCurrentSpeed(), unit.getMaxSpeed(), unit.getDesc_layer());
        }
    }
